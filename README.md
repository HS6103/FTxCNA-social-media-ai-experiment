# Instructions (READ ME FIRST)

## How to download?

1. Click the green "Code" button
2. Click "Download ZIP"
3. Done, unzip the downloaded files

## Final report slides

[Link to Github pages](https://hs6103.github.io/FTxCNA-social-media-ai-experiment/)

## File overview

```markdown
|   final-report.html                   // 7/1 final report slides source code
|   index.html                          // For Github Pages deployment
|   README.md
|
+---data_xlsx_archive                   // Raw data collected during experiment, including engagement, reach,...etc
|       志明AI實驗數據_6_1-6_12.xlsx
|       志明AI實驗數據_6_1-6_15.xlsx
|       志明AI實驗數據_6_1-6_25.xlsx
|       志明AI實驗數據_6_1-6_29.xlsx
|       志明AI實驗數據_6_1-6_30.xlsx    // Final data
|
```

## Contact

+ [蕭煦宸 Simon Hsiao @ CNA Media Lab](https://www.linkedin.com/in/%E7%85%A6%E5%AE%B8-%E8%95%AD-9a879a2b2/)
+ [Email](mailto:simon@gws.cna.com.tw)
